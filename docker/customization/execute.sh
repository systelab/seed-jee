...
# Add the datasource
data-source add --name=SEED --driver-name=mysql --jndi-name=java:/SEED --connection-url="jdbc:mysql://$MYSQL_HOST:$MYSQL_PORT/$MYSQL_DATABASE?useSSL=false&amp;useUnicode=true&amp;characterEncoding=UTF-8" --user-name=$MYSQL_USER --password=$MYSQL_PASSWORD --use-ccm=false --max-pool-size=25 --blocking-timeout-wait-millis=5000 --enabled=true
